  ...
  if(!request.query.code){
    ...
    return superagent.post(GOOGLE_OAUTH_URL)
      ...
      .send({
        ...
        redirect_url: `${process.env.API_URL}/oauth/google`,
      })
      .then(tokenResponse => {
      ...
      .then(openIDResponse => {
        ...
        response.cookie('X-401d21-OAuth-Token', 'This is a Token from our backend');
        response.redirect(process.env.CLIENT_URL);
      })
      .catch(error => {
        console.log('__ERROR__', error.message);
        response.cookies('X-401d21-OAuth-Token', '');
